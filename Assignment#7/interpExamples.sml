...
interp(AST_LET("f", ARROW(INT, INT), AST_FUN("x", INT, AST_APP(AST_SUCC, AST_ID "x")), AST_APP(AST_ID "f", AST_NUM 4)), new_env());

(* "let f : int -> int = (fn x : int => succ x) in (f 4) end" *)
val it = RES_NUM 5 : result

interp(AST_APP(AST_FUN("x", INT, AST_APP(AST_FUN("y", INT, AST_APP (AST_SUCC, AST_ID "x")), AST_NUM 0)), AST_NUM 7), new_env());
